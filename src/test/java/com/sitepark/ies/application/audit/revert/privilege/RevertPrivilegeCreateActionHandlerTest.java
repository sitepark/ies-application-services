package com.sitepark.ies.application.audit.revert.privilege;

import static org.mockito.ArgumentMatchers.any;
import static org.mockito.Mockito.mock;
import static org.mockito.Mockito.verify;
import static org.mockito.Mockito.when;

import com.sitepark.ies.application.privilege.RemovePrivilegesService;
import com.sitepark.ies.audit.core.domain.value.AuditLogTarget;
import com.sitepark.ies.audit.core.service.RevertRequest;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;

class RevertPrivilegeCreateActionHandlerTest {

  private RemovePrivilegesService removePrivilegesService;
  private RevertRequest request;
  private RevertPrivilegeCreateActionHandler handler;

  @BeforeEach
  void setUp() {
    this.removePrivilegesService = mock();
    this.request = mock();

    AuditLogTarget target = mock();
    when(this.request.target()).thenReturn(target);
    when(target.id()).thenReturn("1");
    when(this.request.parentId()).thenReturn("parent-id");

    this.handler = new RevertPrivilegeCreateActionHandler(this.removePrivilegesService);
  }

  @Test
  void testRevertCallsRemovePrivilegesService() {
    this.handler.revert(this.request);

    verify(this.removePrivilegesService).removePrivileges(any());
  }
}

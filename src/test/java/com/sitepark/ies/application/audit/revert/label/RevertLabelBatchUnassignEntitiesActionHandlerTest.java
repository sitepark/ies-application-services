...
class RevertLabelBatchUnassignEntitiesActionHandlerTest {
  ...
  private RevertLabelBatchUnassignEntitiesActionHandler handler;
  ...
  void setUp() {
    ...
    this.handler =
        new RevertLabelBatchUnassignEntitiesActionHandler(
  ...
  @Test
  void testRevertWithEmptyChildIdsDoesNotCallAssignService() {
    when(this.auditLogService.getRecursiveChildIds("log-id")).thenReturn(List.of());

    this.handler.revert(this.request);

    verify(this.assignLabelsToEntitiesService, never()).assignLabelsToEntities(any());
  }

  @Test
  void testRevertCallsAssignLabelsToEntitiesServiceForEachChild() throws IOException {
  ...
  void testRevertWithDeserializationFailureThrowsRevertFailedException() throws IOException {
    ...
    assertThrows(
        ...
        () -> this.handler.revert(this.request),
        "revert() should throw RevertFailedException when deserialization of labelIds fails");
  }
}
